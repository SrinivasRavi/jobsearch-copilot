# Mode: batch — Mass Listing Processing
...
Claude Conductor (claude --chrome --dangerously-skip-permissions)
  ...
  │
  ├─ Listing 2: click next, reads JD + URL
  │    └─► claude -p worker → report .md + PDF + tracker-line
  │
...
batch/
  ...
  logs/                         # One log per listing (gitignored)
...
4. **For each pending URL**:
   ...
   d. Execute via Bash:
      ...
      claude -p --dangerously-skip-permissions \
        ...
        "Process this listing. URL: {url}. JD: /tmp/batch-jd-{id}.txt. Report: {num}. ID: {id}"
      ```
...
- `--max-retries N` — attempts per listing (default: 2)
...
- Each worker is independent: failure in listing #47 doesn't affect others
