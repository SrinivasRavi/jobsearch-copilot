...
Processes listing URLs accumulated in `data/pipeline.md`. The user adds URLs whenever they want, then runs `/career-ops pipeline` to process them all.
...
2. **For each pending URL**:
   ...
   d. **Execute complete auto-pipeline**: A-F Evaluation → Report .md → PDF (if score >= 3.0) → Tracker
...
- **`local:` prefix**: Read the local file. Example: `local:jds/linkedin-backend-swe.md` → read `jds/linkedin-backend-swe.md`
