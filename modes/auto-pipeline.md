...
3. **WebSearch (last resort):** Search role title + company name on secondary portals that index the JD in static HTML.
...
Execute exactly as the `evaluate` mode (read `modes/evaluate.md` for all blocks A-F).
...
**Tone rules:**
- **Confident without arrogance**: "I spent 3 years building S3's storage control plane at 75k host scale — your role is where I want to apply that experience next"
...
- **Relevant experience?** → A quantified proof point. "Built S3's data migration system — 50% efficiency gain, $2M annual savings."
