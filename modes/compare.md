...
For each listing: score on each dimension, weighted total score.
Final ranking + recommendation with time-to-offer considerations.

Ask the user for listings if they're not in context. Can be text, URLs, or references to listings already evaluated in the tracker.
